...
// # 主函数
    ...
    function Class( className, superClass, interfaces, classBody ) {
        ...
        case 3 :
            ...
            if ( Class.isClass( tmp ) || tmp === Object ) {
                superClass = tmp;
                interfaces = null;
            } else {
        ...
        default :
            ...
            if ( Class.isClass( tmp ) || tmp === Object )
                // is class
                superClass = tmp;
            else if ( Class.isInterface( superClass ) ) {
                // is interface
        ...
        var ClassProxy = newClassProxy( classBody, constructorName );
        ...
        // # 实现接口
        checkImplements( ClassProxy, interfaces );
    ...
    function newClassProxy( classBody, constructorName ) {
        function ClassProxy() {
            // 当this是ClassProxy实例或是Class时, 则表示创建实例
            if ( this instanceof arguments.callee || this === instanceFlg ) {
                var o = new classBody();
        ...
        ClassProxy.instance = function() {
            return ClassProxy.apply( instanceFlg, arguments );
        };
    ...
    function Extends( clazz, superClass ) {
        ...
        clazz[ PROTOTYPE ] = new (f || f); // 避免被IDE检索, f绝对存在
 ...
 */
    ...
    function Super1() {
        ...
        if ( superClass === Object )
            ...
            // tmpSuper.__proto__ = superObject; // 暂注释,覆盖__proto__会导致函数无法获取调用者caller
            tmpSuper = null;
...
//        }
    ...
    function doImport( className, successCallback, async, multi ) {
        ...
        var codeWrapFn = cm_classPackMap[ fullClassPath ];
...
// # 代码植入
    ...
    var embedCode = [ '/* [system embedded code] */',
                       // 新增关键字
...
// # 基类
    ...
    Class( 'Class.Object', function () {
        /* [system embedded code] */
        var Super = null, This = null;
        this[ 'Object' || '' ] = function() { // 避免IDE检索成全局Object
            Super();
        };
...
//     * 代码着色
//     * @public
//     * @param {String} code     - 需要着色的代码
//     * @return {String}         - 返回着色后的代码
//     */
//    Class.Compiler.prototype.coloring = function( code ) {
//        return code;
//    };
